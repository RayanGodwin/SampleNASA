...
{
    func buildURLForAstronomyPicturOfTheDay(dateString: String) -> URLRequest?
...
struct PlanetaryURLCreator: PlanetaryURLCreating
{
    ...
    func buildURLForAstronomyPicturOfTheDay(dateString: String) -> URLRequest?
    {
        ...
        if let url = self.urlBuilder.set(scheme: "https").set(host: host).set(path: PlanetaryPathComponents.APOD.description).addQueryItem(name: "api_key", value: demoKey).addQueryItem(name: "date", value: dateString).build()
        {
